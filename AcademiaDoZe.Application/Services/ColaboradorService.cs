...
public class ColaboradorService : IColaboradorService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var logradouro = await _logradouroRepoFactory()
                .ObterPorId(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var colaborador in colaboradores)
        {
            ...
            resultado.Add(colaborador.ToDto(logradouro));
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_logradouroRepoFactory == null)
            return [.. encontrados.Select(c => c.ToDto())];

        var resultado = new List<ColaboradorDto>();

        foreach (var colaborador in encontrados)
        {
            var logradouro = await _logradouroRepoFactory()
                .ObterPorId(
                    colaborador.Endereco.LogradouroId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Já existe um colaborador cadastrado com o CPF {colaboradorDto.Cpf}.");
        }
        ...
        if (!string.IsNullOrWhiteSpace(colaboradorDto.Email))
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Já existe um colaborador cadastrado com o Email {colaboradorDto.Email}.");
            }
        }
        ...
        {
            logradouro = await _logradouroRepoFactory()
                .ObterPorId(
                    colaboradorDto.Endereco.Id,
                    cancellationToken)
                ?? throw new KeyNotFoundException(
                    $"Logradouro com ID " +
                    $"{colaboradorDto.Endereco.Id} não encontrado.");
        }

        var colaborador = colaboradorDto.ToEntity(logradouro);

        var adicionado = await _repoFactory()
            .Adicionar(
                colaborador,
                cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var colaboradorExistente = await _repoFactory()
            ...
            ?? throw new KeyNotFoundException(
                $"Colaborador com ID " +
                $"{colaboradorDto.Id} não encontrado.");
        ...
        {
            throw new InvalidOperationException(
                $"Já existe outro colaborador cadastrado " +
                $"com o CPF {colaboradorDto.Cpf}.");
        }
                ...
                StringComparison.OrdinalIgnoreCase))
        {
                    ...
                    cancellationToken))
            {
                throw new InvalidOperationException(
                    $"Já existe outro colaborador cadastrado " +
                    $"com o Email {colaboradorDto.Email}.");
            }
        }
        ...
        var atualizado = await _repoFactory()
            .Atualizar(
