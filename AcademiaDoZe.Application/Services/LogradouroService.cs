...
public class LogradouroService : ILogradouroService
{
    ...
    public async Task<LogradouroDto?> ObterPorIdAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var logradouro = await _repoFactory()
    ...
    public async Task<IEnumerable<LogradouroDto>> ObterTodosAsync(
        CancellationToken cancellationToken = default)
    {
        var logradouros = await _repoFactory()
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var logradouro = await _repoFactory()
            .ObterPorCep(cepResult.Value!, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var logradouros = await _repoFactory()
            .ObterPorCidade(cidade.Trim(), cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
                ...
                cancellationToken))
        {
            throw new InvalidOperationException(
                $"Já existe um logradouro cadastrado com o CEP {logradouroDto.Cep}.");
        }

        var logradouro = logradouroDto.ToEntity();

        var adicionado = await _repoFactory()
            .Adicionar(logradouro, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var logradouroExistente = await _repoFactory()
            .ObterPorId(logradouroDto.Id, cancellationToken)
        ...
        {
            throw new InvalidOperationException(
                $"Já existe outro logradouro cadastrado com o CEP {logradouroDto.Cep}.");
        }
        ...
        var atualizado = await _repoFactory()
            .Atualizar(logradouroAtualizado, cancellationToken);
